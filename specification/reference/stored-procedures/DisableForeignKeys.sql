...
BEGIN
  ...
  DECLARE getFKs CURSOR FOR
    SELECT CAST(fk.name AS nvarchar) fkname, N'ALTER TABLE [dbo].[' + CAST(o.name AS nvarchar) + N'] NOCHECK CONSTRAINT ' + CAST(fk.name AS nvarchar) + N';' disableFK	   
  ...
  BEGIN
    ...
    PRINT N'Disabling FK ' + @fkname + ' ' + @qry;
