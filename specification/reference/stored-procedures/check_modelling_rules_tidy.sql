...
-- DECLARE @CurrentRelease int = 5
  ...
  AND    1 < (SELECT COUNT(DISTINCT hv2.SubCategoryVID)  
			  ...
			  --JOIN   [TableVersion]					tv2  ON (tvh.TableVID   = tv2.TableVID)
  			  JOIN	 [Header]						h2	 ON (hv2.HeaderID	= h2.HeaderID)
  			  JOIN	 [ModuleVersionComposition] 	mvc2 ON (tvh2.TableVID	= mvc2.TableVID)
              JOIN	 [ModuleVersion]				mv2  ON (mv2.ModuleVID	= mvc2.ModuleVID) 
			  --JOIN   [SubCategoryVersion]			scv2 ON (scv2.SubCategoryVID = hv2.SubCategoryVID)
  			  WHERE  h2.isKey			= 0 
  			  AND	 tvh2.IsAbstract	= 0 
			  ...
			  --AND    tv2.EndReleaseID is NULL 
			  --AND    scv2.EndReleaseID IS NULL
             ) 
...
-- 32 Same TableVersion Code must always belong to one TableID
  INSERT INTO ModelViolations 
  -- DECLARE @CurrentRelease int = 1010000026  
  SELECT DISTINCT 
  ...
  AND    trim(tv.Code) IN
			 (SELECT trim(tv2.Code)
			  ...
			  INNER JOIN ModuleVersion mv2 ON mv2.ModuleVID = mvc2.ModuleVID 
  			  WHERE  (tv2.TableID <> tv.TableID) 
  			  AND	 ( (mv2.EndReleaseID	is NULL AND mv2.StartReleaseID<>9999) OR mv2.EndReleaseID=9999)
             ) 
...
-- 41: A non-Enumrated Category (except 'Not applicable') cannot have associated Items or SubCategories
  ...
  -- 42 An ItemCode (including Property) has to be Unique within its associated Category amongst all ItemCategory assignments
  -- a.ItemCode
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 5    
  SELECT DISTINCT 
		 '6_6'																										AS ViolationCode, 
		 'Duplicate ItemCode within a specific Category'															AS Violation,
		 1																											AS isBlocking,
		 NULL   																									AS TableVID, 
		 NULL																										AS OldTableVID,
		 NULL			    																						AS TableCode, 
  ...
  WHERE 
  it.IsProperty=0 AND
  (1<>(select count(distinct ic2.ItemID) from ItemCategory ic2 where ic2.Code=ic.Code and ic2.CategoryID=ic.CategoryID ))
  ...
  WHERE 
  it.IsProperty=1 AND
  ((pc.EndReleaseID is Null and pc.StartReleaseID<>9999) or pc.EndReleaseID=9999) AND 
  ((ic.EndReleaseID is Null and ic.StartReleaseID<>9999) or ic.EndReleaseID=9999) AND 
  (1<>
  (select count(distinct ic2.ItemID) from ItemCategory ic2 inner join PropertyCategory pc2 on ic2.ItemID=pc2.PropertyID 
    where ic2.Code=ic.Code and 
    ((pc2.EndReleaseID is Null and pc2.StartReleaseID<>9999) or pc2.EndReleaseID=9999) AND 
    ((ic2.EndReleaseID is Null and ic2.StartReleaseID<>9999) or ic2.EndReleaseID=9999)
  )
  )
...
--      (i.e must also have: TableVersion.EndRelease=Null)
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 1010000032
  SELECT DISTINCT 
  ...
  WHERE  (tv.EndReleaseID is Not Null OR tv.StartReleaseID=9999)
  AND    mv.StartReleaseID = @CurrentRelease 
...
-- 46:  Every New ModuleVersions (with StartRelease=CurrentRelease) should contain at least one TableID or TableVID not existing in the ModuleVersionComposiiton of the previous ModuleVersion
  ...
  AND			exists (SELECT mv2.ModuleVID from MODULEvERSION MV2 where mv2.ModuleID=mv.ModuleID AND mv2.ModuleVID<>mv.ModuleVID AND MV2.StartReleaseID<>9999)		    
...
-- 48: Every TableGroup is not recommended to contain in its TableGroupComposition any Table that does not belong to any ModuleVersionComposition
  ...
  AND   t.TableID not in (select mvc.TableID from ModuleVersionComposition mvc 
						  INNER JOIN ModuleVersion mv on mv.ModuleVID=mvc.ModuleVID 
   					      WHERE mv.EndReleaseID is Null or mv.EndReleaseID=9999
						 )
				*/		   

-- 49: An Enumrated Category must have one Default Item associated with it
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 1010000007  -- 5
    SELECT DISTINCT 
		 '6_9'																										AS ViolationCode, 
		 'Enumerated Category has not default item or More than One default items associated with it'				AS Violation,
		 1																											AS isBlocking,
		 NULL   																									AS TableVID, 
		 NULL																										AS OldTableVID,
		 NULL			    																						AS TableCode, 
  ...
  FROM   [Category] c 
  WHERE 
  c.IsEnumerated=1 AND (c.Code not in ('_PR', '_TE', 'AS')  ) 
  AND 
  ...
  /* DEACTIVATE AS IS INCLUDED WITHIN 6_9 
-- 50: An Enumrated Category must have ONLY ONE Default Item associated with it
  ...
  ( (1 < (select count(distinct ic.ItemID) from ItemCategory ic where ic.CategoryID=c.CategoryID  and ic.IsDefaultItem=1))  ) 
...
-- 51_1: Framework
  ...
  -- the constraint below shows that framework is new
  not exists (SELECT mv.* FROM ModuleVersion mv inner join Module m on mv.ModuleID=m.moduleID 
			  WHERE m.FrameworkID=f.FrameworkID and (mv.StartReleaseID<>@CurrentRelease) AND (mv.StartReleaseID<>9999)) AND 
...
-- 51_9: SubCategory
  ...
  (NOT EXISTS (SELECT * FROM SubCategoryVersion scv WHERE scv.SubCategoryID=sc.SubCategoryID AND scv.StartReleaseID<>@CurrentRelease  AND scv.StartReleaseID<>9999))
...
-- 51_10: Category
  ...
  (NOT EXISTS (SELECT * FROM SubCategoryVersion scv inner join Subcategory sc on scv.SubCategoryID=sc.SubCategoryID WHERE sc.CategoryID=c.CategoryID AND scv.StartReleaseID<>@CurrentRelease AND scv.StartReleaseID<>9999)) AND 
  (NOT EXISTS (SELECT * FROM SuperCategoryComposition scc WHERE (scc.CategoryID=c.CategoryID or scc.SuperCategoryID=c.CategoryID) AND scc.StartReleaseID<>@CurrentRelease AND scc.StartReleaseID<>9999)) AND 
  (NOT EXISTS (SELECT * FROM ItemCategory ic WHERE ic.CategoryID=c.CategoryID AND ic.StartReleaseID<>@CurrentRelease AND ic.StartReleaseID<>9999)) AND 
  (NOT EXISTS (SELECT * FROM PropertyCategory pc WHERE pc.CategoryID=c.CategoryID AND pc.StartReleaseID<>@CurrentRelease AND pc.StartReleaseID<>9999))
...
-- 51_11: Operation
  ...
  WHERE   charindex(' ',trim(op.Code))>0 AND  
		 (NOT EXISTS (SELECT * FROM OperationVersion opv WHERE opv.OperationID=op.OperationID AND opv.StartReleaseID<>@CurrentRelease AND opv.StartReleaseID<>9999))
...
-- 53: Each Table is recommended to belong to one and only one TableGroup of type 'templateGroup'
  INSERT INTO ModelViolations 
  -- DECLARE @CurrentRelease int = 1010000026
  SELECT DISTINCT 
		 '1_11'																										AS ViolationCode, 
		 'Table does not belong to one and only one TableGroup of type templateGroup			'					AS Violation,
		 1																											AS isBlocking,
		 tv.TableVID   																								AS TableVID, 
		 NULL																										AS OldTableVID,
		 tv.Code			    																					AS TableCode, 
  ...
  WHERE 
  mv.StartReleaseID=@CurrentRelease AND t.IsAbstract=0 
  AND   1 <> (select isnull(count(distinct tgc2.TableGroupID),0) 
                           from TableGroupComposition tgc2 
						   INNER JOIN tableGroup tg2 on tg2.TableGroupID=tgc2.TableGroupID  
						   WHERE tgc2.TableID=tv.TableID and tg2.type='templateGroup' 
						     and (tgc2.EndReleaseID is Null and tgc2.StartReleaseID<>9999 OR tgc2.EndReleaseID=9999) 
							 and (tg2.EndReleaseID is Null and tg2.StartReleaseID<>9999 OR tg2.EndReleaseID=9999)
						 )


-- SOS!!! LATER MAKE THIS RULE isBlocking=1
-- SOS!!! THINK TO EXCLUDE PLAYGROUND MODULES FROM HERE
-- 54: All Tables belonging to the same TableGroup of type 'templateGroup' must be associated to the same Module in any active ModuleVersion
  INSERT INTO ModelViolations 
  -- DECLARE @CurrentRelease int = 101000067
  SELECT DISTINCT 
		 ...
		 NULL																										AS NewAspect
  FROM   [TableGroup]            tg 
   WHERE ((tg.EndReleaseID is Null and tg.StartReleaseID<>9999) OR tg.EndReleaseID=9999) 
  AND tg.type='templateGroup'
  AND EXISTS (select mv.ModuleVID from ModuleVersion mv 
							WHERE   mv.StartReleaseID=@CurrentRelease AND 
									(SELECT isNull(count(distinct tgc.TableID),0) FROM ModuleVersionComposition mvc 
...
-- 55: Every active Table must belong to at least onee TableGroup of type 'templateGroup'
  INSERT INTO ModelViolations 
  -- DECLARE @CurrentRelease int = 10000026
  SELECT DISTINCT 
...
WHERE (scv.EndReleaseID is NULL AND scv.StartReleaseID=@CurrentRelease) AND ic.EndReleaseID is NULL 
--AND   scv.StartReleaseID = @CurrentRelease 
AND NOT 
(
...
--     EXCEPTIONS WHERE WE LEAVE OFF: New Modules with ModuleVersions ONLY in Playground Release; there we do not care
INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 5
    SELECT DISTINCT 
		 '1_15'																										AS ViolationCode, 
		 'ItemCategory or PropertyCategory have changed with impact in: "'+sq.source+'". But no New ModuleVersion for Module:'+sq.Code+' has been created'					AS Violation,
		 0																											AS isBlocking,
		 NULL																										AS TableVID, 
		 NULL																										AS OldTableVID,
		 sq.TableCode																								AS TableCode, 
		 NULL																										AS HeaderID, 
		 sq.HeaderCode																								AS HeaderCode, 
		 NULL																										AS HeaderVID, 
		 NULL																										AS OldHeaderVID, 
		 NULL																										AS KeyHeader, 
		 sq.Direction																								AS HeaderDirection, 
		 sq.PropertyID																								AS HeaderPropertyID, 
		 icpr.Code																									AS HeaderPropertyCode, 
		 NULL				  																						AS HeaderSubcategoryID, 
		 NULL																										AS HeaderSubcategoryName, 
		 NULL																										AS HeaderContextID, 
  	     NULL			      																						AS CategoryID, 
...
FROM
(
-- Contexts of HeaderVersions Property
(SELECT mv.*, ic.ItemID, pc.PropertyID, ic.StartReleaseID as ItemStartRelease, pc.StartReleaseID as PropertyStartRelease
 ,tv.Code as TableCode, h.Direction as Direction, hv.Code as HeaderCode, 'header_context' as source 
FROM moduleversion mv 
...
WHERE (mv.EndReleaseID is NULL AND mv.StartReleaseID<>9999 OR mv.EndReleaseID=9999) AND ic.EndReleaseID is NULL AND pc.EndReleaseID is NULL AND
(ic.StartReleaseID = @CurrentRelease OR pc.StartReleaseID=@CurrentRelease) 
AND mv.StartReleaseID <> @CurrentRelease AND exists (select mv2.ModuleVID from ModuleVersion mv2 where mv2.ModuleID=mv.ModuleID and mv2.StartReleaseID<>9999) 
)
UNION
-- Contexts of TableVersions Property
(SELECT mv.*, ic.ItemID, pc.PropertyID, ic.StartReleaseID as ItemStartRelease, pc.StartReleaseID as PropertyStartRelease
,tv.Code as TableCode, '' as Direction, '' as HeaderCode, 'table_context' as source 
FROM moduleversion mv 
INNER JOIN moduleversioncomposition mvc ON mv.ModuleVID = mvc.ModuleVID 
INNER JOIN TableVersion tv ON tv.TableVID = mvc.TableVID 
...
(ic.StartReleaseID = @CurrentRelease OR pc.StartReleaseID=@CurrentRelease) 
AND mv.StartReleaseID <> @CurrentRelease 
AND exists (select mv2.ModuleVID from ModuleVersion mv2 where mv2.ModuleID=mv.ModuleID and mv2.StartReleaseID<>9999) 
)
UNION
-- Contexts of VariableVersions Property
(SELECT mv.*, ic.ItemID, pc.PropertyID, ic.StartReleaseID as ItemStartRelease, pc.StartReleaseID as PropertyStartRelease
 ,tv.Code as TableCode, '' as Direction, '' as HeaderCode, 'variable_context' as source 
FROM moduleversion mv 
INNER JOIN moduleversioncomposition mvc ON mv.ModuleVID = mvc.ModuleVID 
INNER JOIN TableVersion tv ON tv.TableVID = mvc.TableVID 
...
(SELECT mv.*, ic.ItemID, pc.PropertyID, ic.StartReleaseID as ItemStartRelease, pc.StartReleaseID as PropertyStartRelease
,tv.Code as TableCode, h.Direction as Direction, hv.Code as HeaderCode, 'header_property' as source 
FROM moduleversion mv 
INNER JOIN moduleversioncomposition mvc ON mv.ModuleVID = mvc.ModuleVID 
INNER JOIN TableVersion tv ON tv.TableVID = mvc.TableVID 
INNER JOIN TableVersionHeader tvh ON tvh.TableVID = tv.TableVID 
...
(SELECT mv.*, ic.ItemID, pc.PropertyID, ic.StartReleaseID as ItemStartRelease, pc.StartReleaseID as PropertyStartRelease 
,tv.Code as TableCode, '' as Direction, '' as HeaderCode, 'table+property' as source 
FROM moduleversion mv 
INNER JOIN moduleversioncomposition mvc ON mv.ModuleVID = mvc.ModuleVID 
INNER JOIN TableVersion tv ON tv.TableVID = mvc.TableVID 
...
(SELECT mv.*, ic.ItemID, pc.PropertyID, ic.StartReleaseID as ItemStartRelease, pc.StartReleaseID as PropertyStartRelease 
,tv.Code as TableCode, '' as Direction, '' as HeaderCode, 'variable_property' as source 
FROM moduleversion mv 
INNER JOIN moduleversioncomposition mvc ON mv.ModuleVID = mvc.ModuleVID 
INNER JOIN TableVersion tv ON tv.TableVID = mvc.TableVID 
INNER JOIN TableVersionCell tvc ON tv.TableVID = tvc.TableVID 
...
UNION
-- Contexts of HeaderVersions Item
(SELECT mv.*, ic.ItemID, -1 as PropertyID, ic.StartReleaseID as ItemStartRelease, -1 as PropertyStartRelease 
,tv.Code as TableCode, h.Direction as Direction, hv.Code as HeaderCode, 'header_context_item' as source 
FROM moduleversion mv 
INNER JOIN moduleversioncomposition mvc ON mv.ModuleVID = mvc.ModuleVID 
INNER JOIN TableVersion tv ON tv.TableVID = mvc.TableVID 
INNER JOIN TableVersionHeader tvh ON tvh.TableVID = tv.TableVID 
...
INNER JOIN Header h ON hv.HeaderID = h.HeaderID 
INNER JOIN ContextComposition cc ON cc.ContextID = hv.ContextID 
INNER JOIN ItemCategory ic ON ic.ItemID = cc.ItemID 
WHERE (mv.EndReleaseID is NULL AND mv.StartReleaseID<>9999 OR mv.EndReleaseID=9999) AND ic.EndReleaseID is NULL AND
(ic.StartReleaseID = @CurrentRelease) 
AND mv.StartReleaseID <> @CurrentRelease
AND exists (select mv2.ModuleVID from ModuleVersion mv2 where mv2.ModuleID=mv.ModuleID and mv2.StartReleaseID<>9999) 
)
UNION
-- Contexts of TableVersions Item
(SELECT mv.*, ic.ItemID, -1 as PropertyID, ic.StartReleaseID as ItemStartRelease, -1 as PropertyStartRelease 
,tv.Code as TableCode, '' as Direction, '' as HeaderCode, 'table_context_item' as source 
FROM moduleversion mv 
INNER JOIN moduleversioncomposition mvc ON mv.ModuleVID = mvc.ModuleVID 
INNER JOIN TableVersion tv ON tv.TableVID = mvc.TableVID 
INNER JOIN ContextComposition cc ON cc.ContextID = tv.ContextID 
...
UNION
-- Contexts of VariableVersions Item
(SELECT mv.*, ic.ItemID, -1 as PropertyID, ic.StartReleaseID as ItemStartRelease, -1 as PropertyStartRelease 
,tv.Code as TableCode, '' as Direction, '' as HeaderCode, 'variable_context_item' as source 
FROM moduleversion mv 
INNER JOIN moduleversioncomposition mvc ON mv.ModuleVID = mvc.ModuleVID 
INNER JOIN TableVersion tv ON tv.TableVID = mvc.TableVID 
INNER JOIN TableVersionCell tvc ON tv.TableVID = tvc.TableVID 
...
UNION
-- SubcategoryItems of HeaderVersions
(SELECT mv.*, ic.ItemID, -1 as PropertyID, ic.StartReleaseID as ItemStartRelease, -1 as PropertyStartRelease 
,tv.Code as TableCode, h.Direction as Direction, hv.Code as HeaderCode, 'header_subcategory_item' as source 
FROM moduleversion mv 
INNER JOIN moduleversioncomposition mvc ON mv.ModuleVID = mvc.ModuleVID 
INNER JOIN TableVersion tv ON tv.TableVID = mvc.TableVID 
INNER JOIN TableVersionHeader tvh ON tvh.TableVID = tv.TableVID 
INNER JOIN HeaderVersion hv ON hv.HeaderVID = tvh.HeaderVID 
INNER JOIN Header h ON hv.HeaderID = h.HeaderID 
INNER JOIN SubCategoryItem sci ON sci.SubCategoryVID = hv.SubCategoryVID 
INNER JOIN ItemCategory ic ON ic.ItemID = sci.ItemID 
WHERE (mv.EndReleaseID is NULL AND mv.StartReleaseID<>9999 OR mv.EndReleaseID=9999) AND ic.EndReleaseID is NULL AND
(ic.StartReleaseID = @CurrentRelease) 
AND mv.StartReleaseID <> @CurrentRelease
...
LEFT JOIN ItemCategory icin on icin.ItemID=sq.ItemID and icin.EndReleaseID is Null 
LEFT JOIN ItemCategory icpr on icpr.ItemID=sq.PropertyID and icpr.EndReleaseID is Null 
...
-- 70. A Property Name has to be Unique within its own Category
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 5
  SELECT DISTINCT 
		 '6_25'																										AS ViolationCode, 
		 left('Property Name is not essentially Unique within its Category: "'+trim(it.Name),160)+'"'				AS Violation,
		 1																											AS isBlocking,
		 NULL																										AS TableVID, 
		 NULL																										AS OldTableVID,
		 NULL																										AS TableCode, 
  ...
  INNER JOIN Property p ON it.ItemID=p.PropertyID  
  INNER JOIN PropertyCategory pc ON pc.PropertyID=p.PropertyID  
  INNER JOIN Category c ON pc.CategoryID=c.CategoryID 
  WHERE  
  it.IsProperty = 1 
  ...
  AND NOT EXISTS (SELECT * FROM 
			  ItemCategory ic2 
			  WHERE ic2.StartReleaseID <> @CurrentRelease  
			  AND ic2.ItemID=ic.ItemID 
			 ) 
  AND EXISTS (SELECT * FROM 
			  PropertyCategory pc2 INNER JOIN Item it2 ON it2.ItemID = pc2.PropertyID 
			  WHERE pc2.EndReleaseID is NULL 
			  AND it2.ItemID<>it.ItemID 
			  AND it2.isProperty = 1 
			  AND pc2.CategoryID=pc.CategoryID 
			  AND 
			  replace(replace(replace(replace(replace(it2.Name,' ',''),'.',''),')',''),'(',''),'_','') 
            = replace(replace(replace(replace(replace(it.Name ,' ',''),'.',''),')',''),'(',''),'_','')       
			 ) 
    
  
  

-- 71. An Item Name created in current release has to be essentially Unique within its Category (by ignoring these characters : "-", "–", "(", ")",".",  " ")
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 5
    SELECT DISTINCT 
		 '6_26'																										AS ViolationCode, 
		 left('Item Name is not essentially Unique within its Category itself: "'+trim(it.Name),170)+'"'			AS Violation,
		 1																											AS isBlocking,
		 NULL																										AS TableVID, 
		 NULL																										AS OldTableVID,
		 NULL																										AS TableCode, 
...
--DECLARE @CurrentRelease int = 4  
  ...
  -- 82: SubCategory Code within a Category has to be unique
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 4
    SELECT DISTINCT 
		 '6_29'																										AS ViolationCode, 
		 'Duplicate SubCategory Code within the same Category'														AS Violation,
		 1																											AS isBlocking,
		 sc.SubCategoryID																							AS TableVID, 
		 NULL																										AS OldTableVID,
		 left(trim(sc.Code) + ' (SUBCATEGORY from Category '+c.Code+')',40)											AS TableCode, 
  ...
  WHERE   sc.Code is not Null AND 
  (EXISTS (SELECT * FROM SubCategory sc2 WHERE sc2.SubCategoryID<>sc.SubCategoryID AND sc2.Code=sc.Code AND sc2.CategoryID=sc.CategoryID)) AND 
  (EXISTS (SELECT * FROM SubCategoryVersion scv WHERE scv.SubCategoryID=sc.SubCategoryID AND scv.StartReleaseID=@CurrentRelease))



  -- 83: Category Code has to be unique
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 4
    SELECT DISTINCT 
		 '6_30'																										AS ViolationCode, 
		 'Duplicate Category Code'																					AS Violation,		 
		 1																											AS isBlocking,
		 NULL																										AS TableVID, 
		 NULL																										AS OldTableVID,
		 NULL																										AS TableCode, 
		 ...
		 NULL																										AS NewAspect
  FROM  Category c 
  WHERE     (EXISTS (SELECT * FROM Category c2 WHERE c2.CategoryID<>c.CategoryID AND c2.Code=c.Code ))



  
-- 84: Every Item code must start with a letter and there should not be aby space characters in the Item Code
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 1010000054
    SELECT DISTINCT 
		 '6_31'																										AS ViolationCode, 
		 'Item code does not start with a letter or there are spaces within the Item code'							AS Violation,
		 1																											AS isBlocking,
		 NULL													AS TableVID, 
		 NULL													AS OldTableVID,
		 NULL													AS TableCode, 
  ...
  AND	ic.EndReleaseID IS NULL
  AND	ic.StartReleaseID = @CurrentRelease 
  AND	(((left(upper(ic.Code),1) not between 'A' and 'Z') and (left(ic.Code,1)<>'_')) or charindex(' ',ic.Code)>0 ) 
  




-- 85: All technical Tables that belong to nthe same Abstract table have to belong to the  same TableGroup of type 'templateGroup'
  INSERT INTO ModelViolations 
  -- DECLARE @CurrentRelease int = 1010000026
  SELECT DISTINCT 
		 '1_16'																										AS ViolationCode, 
		 'Technical Table belongs to a different Template Group than another technical Table that emanates from the same Abstract Table as the current one: '
		 +tv2.Code  AS Violation,
		 1																											AS isBlocking,
		 tv.TableVID   																								AS TableVID, 
		 NULL																										AS OldTableVID,
		 tv.Code			    																					AS TableCode, 
  ...
  INNER JOIN moduleversioncomposition mvc0 ON mvc0.TableVID=tv.TableVID 
  INNER JOIN ModuleVersion mv0			   ON mvc0.ModuleVID=mv0.ModuleVID 
  INNER JOIN TableGroupComposition tgc	   ON tgc.TableID = tv.TableID 
  INNER JOIN TableGroup tg				   ON tg.TableGroupID=tgc.TableGroupID 
  INNER JOIN TableVersion tv2			   ON tv2.AbstractTableID=TV.AbstractTableID 
  inner joiN TableGroupComposition tgc2    ON tv2.TableID=tgc2.TableID 
  INNER JOIN tableGroup tg2 on tg2.TableGroupID=tgc2.TableGroupID  
  WHERE tgc.StartReleaseID = @CurrentRelease 
  AND tg.Type='templateGroup'
  AND tv.EndReleaseID is Null 
  AND mv0.StartReleaseID = @CurrentRelease 
  AND t.IsAbstract = 0 
  AND tv.AbstractTableID is NOT NULL
  and tgc2.TableGroupID<>tgc.TableGroupID 
  and tg2.type='templateGroup' 
  and tg2.EndReleaseID is Null  
  and tgc2.EndReleaseID is Null 
  and tv2.EndReleaseID is Null 
  and tv2.TableID<>tv.TableID 


-- 86. Each SubCategoryVersion that starts on Current Release, must have at least one corresponding SubCategoryItem
 INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 5
    SELECT DISTINCT 
		 '6_32'																										AS ViolationCode, 
		 'SubCategory has the same Name as another SubCategory of the Same Category'								AS Violation,
		 1																											AS isBlocking,
		 sc.SubCategoryID																							AS TableVID, 
		 NULL																										AS OldTableVID,
		 left('SubCategory Code:'+trim(sc.Code),40)																	AS TableCode, 
  ...
  WHERE  
  scv.EndReleaseID is NULL and scv.StartReleaseID = @CurrentRelease 
  AND NOT EXISTS (Select * FROM SUbCategoryVersion scv2 WHERE scv2.SubCategoryID=scv.SubCategoryID and scv2.StartReleaseID<>@CurrentRelease)
  AND (EXISTS (SELECT * FROM SubCategory sc2 WHERE sc2.SubCategoryID<>sc.SubCategoryID and sc2.Name=sc.Name and sc2.CategoryID=sc.CategoryID))

   

-- 87. Each SubCategoryVersion that starts on Current Release, must have at least one corresponding SubCategoryItem
 INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 5
    SELECT DISTINCT 
		 '6_33'																										AS ViolationCode, 
		 'SubCategoryVersion was created in this Release but does not contain any SubCategoryItem'					AS Violation,
		 1																											AS isBlocking,
		 sc.SubCategoryID																							AS TableVID, 
		 NULL																										AS OldTableVID,
		 left('SubCategory Code:'+trim(sc.Code),40)																	AS TableCode, 
  ...
  WHERE  
  scv.EndReleaseID is NULL and scv.StartReleaseID = @CurrentRelease 
  AND NOT EXISTS (Select * FROM SUbCategoryItem sci WHERE sci.SubCategoryVID=scv.SubCategoryVID)
 



-- 88. Every Active TableVersion (with EndRelease=Null) must have Unique Name 
-- Attention: Several duplicates discovered after relaxing condition
  INSERT INTO ModelViolations 
  -- DECLARE @CurrentRelease int = 101000033  
  SELECT DISTINCT 
		 '1_17'																										AS ViolationCode, 
		 'Duplicate Name: "'+left(tv.Code +': '+tv.Name,100)+'"... of TableVersion that belongs to a Module changing in Current Release with: '
		 +tv2.Code AS Violation,
		 1																											AS isBlocking,
		 tv.TableVID   																								AS TableVID, 
		 NULL																										AS OldTableVID,
  ...
  FROM   [TableVersion] 			tv 
   ...
   JOIN [ModuleVersionComposition] mvc on mvc.TableVID = tv.TableVID 
   JOIN [ModuleVersion] mv on mv.ModuleVID = mvc.ModuleVID 
   JOIN [TableVersion]  tv2 ON trim(tv.Name)=trim(tv2.Name)
   JOIN [Table] t2 on t2.TableID = tv2.TableID 
   JOIN ModuleVersionComposition mvc2 ON tv2.TableVID=mvc2.TableVID 
   JOIN ModuleVersion mv2 ON mv2.ModuleVID = mvc2.ModuleVID 
  WHERE mv.StartReleaseID = @CurrentRelease 
  AND (tv2.TableID < tv.TableID) 
  AND (t.isAbstract = t2.isAbstract) 
  AND ((mv2.EndReleaseID is NULL AND mv2.StartReleaseID<>9999) OR mv2.EndReleaseID=9999) 

  
   
-- 89: These characters are not allowed to be contained in TableGroup Code: "-", "–", "(", ")"," "
  INSERT INTO ModelViolations 
  -- DECLARE @CurrentRelease int = 5
  SELECT DISTINCT 
		 '1_18'																										AS ViolationCode, 
		 'TableGroup Code contains one of these illegal characters: "-", "–", "(", ")", ".", " "'	AS Violation,
		 1																											AS isBlocking,
		 NULL   																								AS TableVID, 
		 NULL																										AS OldTableVID,
		 'TableGroup Code: "'+trim(left(tg.Code,230))+'"'															AS TableCode, 
		 NULL																										AS HeaderID, 
		 NULL																							            AS HeaderCode, 
		 NULL																										AS HeaderVID, 
		 NULL																										AS OldHeaderVID, 
		 ...
		 NULL																										AS NewAspect
  FROM   TableGroup tg		
  WHERE tg.StartReleaseID = @CurrentRelease
  AND tg.Type='templateGroup'
  AND 
  (
  charindex('-',tg.Code)>0 OR 
  charindex('–',tg.Code)>0 OR 
  charindex('(',tg.Code)>0 OR 
  charindex(')',tg.Code)>0 OR 
  charindex('.',tg.Code)>0 OR 
 ...
 -- 90. When we Copy-Paste one Table to another, then the destination table has to maintain unchanged the open Row/Column/Sheet settings of the source table
  INSERT INTO ModelViolations 
  -- DECLARE @CurrentRelease int = 5  
  SELECT DISTINCT 
		 '1_19'																										AS ViolationCode, 
		 'Destination table has different Open Row/Column/Sheet settings than the source table'						AS Violation,
		 1																											AS isBlocking,
		 tv.TableVID   																								AS TableVID, 
		 NULL																										AS OldTableVID,
		 tv.Code			    																					AS TableCode, 
  ...
  FROM   [TableVersion] 			tv 
   ...
   JOIN [ModuleVersionComposition] mvc on mvc.TableVID = tv.TableVID 
   JOIN [ModuleVersion] mv on mv.ModuleVID = mvc.ModuleVID
   JOIN [Aux_CellMapping] cm ON cm.NewTableVID=tv.TableVID 
   JOIN [TableVersion]			   tv2  on tv2.TableVID = cm.OldTableVID 
   JOIN [Table]					   t2   on t2.TableID = tv2.TableID 
  WHERE 
  mv.StartReleaseID = @CurrentRelease
  and   (t.HasOpenColumns<>t2.HasOpenColumns OR t.HasOpenRows<>t2.HasOpenRows OR t.HasOpenSheets<>t2.HasOpenSheets) 
    

-- 91. A SubCategory Updated in CurrentRelease cannot contain exactly the same Items as another existing active SubCategory
 INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 1010000026
        SELECT --DISTINCT 
		 '6_34'																										AS ViolationCode, 
		 'SubCategory: '+trim(left(sc.Code,20))+' Updated in release: '+r.Code+' contains exactly the same Items as the existing active SubCategory: '
		 +trim(left(sc2.Code,20))+' updated in Release: '+r2.Code		AS Violation,
		 1																											AS isBlocking,
		 NULL																										AS TableVID, 
		 NULL																										AS OldTableVID,
		 NULL																										AS TableCode, 
  ...
  INNER JOIN Category c on c.CategoryID=sc.CategoryID 
  inner join SubCategoryVersion SCV on SC.SubCategoryID=SCV.SubCategoryID 
  inner join SubCategory	 sc2 on sc2.CategoryID=c.CategoryID 
  inner join SubCategoryVersion scv2 on scv2.SubCategoryID=sc2.SubCategoryID  
  inner join Release r ON scv.StartReleaseID=r.ReleaseID 
  inner join Release r2 ON scv2.StartReleaseID=r2.ReleaseID 
  WHERE  
    scv2.subcategoryvid<scv.subcategoryvid AND 
	sc.SubCategoryID<>sc2.SubCategoryID AND 
	((scv.EndReleaseID is NULL and SCV.StartReleaseID<>9999) or SCV.EndReleaseID=9999)
	and scv.StartReleaseID = @CurrentRelease 
	and ((scv2.EndReleaseID is NULL and SCV2.StartReleaseID<>9999) or SCV2.EndReleaseID=9999)
   AND (SELECT count(*) FROM SUbCategoryItem sci WHERE sci.SubCategoryVID=scv.SubCategoryVID) = 
  (SELECT count(*) FROM SUbCategoryItem sci INNER JOIN SubCategoryItem sci2 on sci.ItemID=sci2.ItemID 
    WHERE sci.SubCategoryVID=scv.SubCategoryVID AND sci2.SubCategoryVID=scv2.SubCategoryVID)
  AND (SELECT count(*) FROM SUbCategoryItem sci WHERE sci.SubCategoryVID=scv.SubCategoryVID) = 
      (SELECT count(*) FROM SUbCategoryItem sci2 WHERE sci2.SubCategoryVID=scv2.SubCategoryVID)
  ORDER BY r.Code desc, sc.Code 




 -- 92.	An Abstract Header cannot have any Property or Context assigned to it
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 5
  SELECT DISTINCT 
		 '3_16'												AS ViolationCode, 
		 'An Abstract Header cannot have any Property or Context assigned to it' AS Violation,
		 1													AS isBlocking,
		 tv.TableVID										AS TableVID, 
		 NULL												AS OldTableVID,
		 tv.Code											AS TableCode, 
  ...
  LEFT OUTER JOIN itemCategory ic ON hv.PropertyID=ic.ItemID 
  WHERE t.IsAbstract    = 0
  AND   tvh.IsAbstract  = 1
  AND	hv.EndReleaseID is NULL 
  AND   mv.StartReleaseID = @CurrentRelease 
  AND   (hv.ContextID is not null OR hv.PropertyID is not null)
  ORDER BY tv.TableVID;

  
  
-- 93. A Property Name has to be Unique across all Categories
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 1010000067
  SELECT DISTINCT 
		 '6_35'																										AS ViolationCode, 
		 left('Property Name is not essentially Unique acrossall Categories: "'+trim(it.Name),160)+'"'				AS Violation,
		 0																											AS isBlocking,
		 NULL																										AS TableVID, 
		 NULL																										AS OldTableVID,
		 NULL																										AS TableCode, 
  ...
  AND EXISTS (SELECT * FROM 
			  PropertyCategory pc2 INNER JOIN Item it2 ON it2.ItemID = pc2.PropertyID 
			  WHERE (pc2.EndReleaseID is NULL or pc2.EndReleaseID=9999)
			  AND it2.ItemID<>it.ItemID 
			  AND it2.isProperty = 1 
			  AND 
			  replace(replace(replace(replace(replace(it2.Name,' ',''),'.',''),')',''),'(',''),'_','') 
...
-- 94:  For every New TableVersion in the CurrentRelease, all relevant Modules that employ that TableID in their composiiton must have new ModuleVersion in this CurrentRelease  
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 1010000026
  SELECT DISTINCT 
		 '1_20'																										AS ViolationCode, 
		 'Table with New TableVersion is employed by Old active ModuleVersion'										AS Violation,
		 1																											AS isBlocking,
		 tv.TableVID																								AS TableVID, 
		 NULL																										AS OldTableVID,
		 tv.Code		    																						AS TableCode, 
  ...
  FROM [TableVersion]  tv 
  JOIN [Table]         t  ON (t.TableID=tv.TableID)
  JOIN [ModuleVersionComposition] mvc on mvc.TableID = t.TableID 
  JOIN [ModuleVersion] mv on mv.ModuleVID = mvc.ModuleVID
  WHERE    tv.StartReleaseID = @CurrentRelease 
  and      ((mv.EndReleaseID is NULL AND mv.StartReleaseID<>9999) or mv.EndReleaseID=9999) 
  AND      mv.StartReleaseID <> @CurrentRelease 



-- 95: For any table in the TableGroup with StartRelease = draft release is not allowed to exist within any tablegroupcomposition that has tgc.StartRelease=9999 (playground)
  INSERT INTO ModelViolations 
  -- DECLARE @CurrentRelease int = 1010000026
  SELECT DISTINCT 
		 '1_21'																										AS ViolationCode, 
		 'Table exists within TableGroup starting in draft Release and has TableGroupComposition.StartRelease=9999'		AS Violation,
		 1																											AS isBlocking,
		 tv.TableVID   																								AS TableVID, 
		 NULL																										AS OldTableVID,
		 tv.Code			    																					AS TableCode, 
		 ...
		 NULL																										AS NewAspect
  FROM  
  [TableGroupComposition] tgc 
  INNER JOIN [TableGroup]            tg  ON tgc.TableGroupID=tg.TableGroupID 
  INNER JOIN [TableVersion]			tv ON tgc.TableID=tv.TableID 	
  WHERE 
  tg.StartReleaseID = @CurrentRelease AND 
  tgc.StartReleaseID = 9999 AND 
  tv.EndReleaseID is NULL 


/*  


-- 96:  For every Table Association created in currentRelease, there does not exist another TableAssociation between the same ParentTableVID and ChildTableVID and Same Headers that is either Active (EndRelease=Null) or has EndRelease=CurrentRelease
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 9999   --1010000026
  SELECT DISTINCT 
		 '1_22'																										AS ViolationCode, 
		 'Replicated TableAssociation '+ta1.Name+' with existing: '+ta2.Name+
		 '. Please remove current Table Association. (Only if EndRelease=CurrentRelease: Please re-open an identical asscoiation already existing between same Tables and Columns that you just closed)'										
																													AS Violation,
		 1																											AS isBlocking,
		 tv1.TableVID																								AS TableVID, 
		 NULL																										AS OldTableVID,
		 tv1.Code		    																						AS TableCode, 
		 NULL																										AS HeaderID, 
		 NULL																										AS HeaderCode,	
		 NULL																										AS HeaderVID, 
		 NULL																										AS OldHeaderVID, 
		 ...
		 NULL																										AS VVEndReleaseID,
		 NULL																										AS NewAspect
  FROM [TableAssociation] ta1 
  JOIN [TableVersion]  tv1 ON ta1.ParentTableVID=tv1.TableVID 
  JOIN [TableAssociation]  ta2 ON ta2.ParentTableVID=ta1.ParentTableVID AND ta1.ChildTableVID=ta2.ChildTableVID 
  JOIN [TableVersion]  tv2 ON ta2.ParentTableVID=tv2.TableVID 
  WHERE ta2.AssociationID<ta1.AssociationID AND TA1.StartReleaseID=@CurrentRelease AND (ta2.EndReleaseID is NULL OR ta2.EndReleaseID=@CurrentRelease)    
  AND NOT EXISTS   (SELECT khm2.ForeignKeyHeaderID 
					FROM KeyHeaderMapping khm1 
					JOIN KeyHeaderMapping khm2 ON khm1.PrimaryKeyHeaderID=khm1.PrimaryKeyHeaderID AND khm1.AssociationID=ta1.AssociationID AND khm2.AssociationID=ta2.AssociationID 
					WHERE khm2.ForeignKeyHeaderID<>khm1.ForeignKeyHeaderID)
					

-- 97:  The categories of all Properties in KeyHeaderMapping need always to be maintained the same. 
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 9999   --1010000026
  SELECT DISTINCT 
		 '1_23'																										AS ViolationCode, 
		 'Inconsistent Categories of Properties in TableAssociation '+ta.Name+' between linked Properties'			AS Violation,
		 1																											AS isBlocking,
		 tv.TableVID																								AS TableVID, 
		 NULL																										AS OldTableVID,
		 tv.Code		    																						AS TableCode, 
		 ...
		 NULL																										AS HeaderDirection, 
		 hv.PropertyID																								AS HeaderPropertyID, 
		 'Parent:'+ic.Code																					AS HeaderPropertyCode, 
		 NULL				  																						AS HeaderSubcategoryID, 
		 NULL																										AS HeaderSubcategoryName, 
		 NULL																										AS HeaderContextID, 
  	     c.CategoryID 																								AS CategoryID, 
		 ...
		 NULL																										AS VVEndReleaseID,
		 NULL																										AS NewAspect
  FROM [TableAssociation] ta 
  JOIN [TableVersion]  tv ON ta.ParentTableVID=tv.TableVID 
  JOIN [TableVersion]  tv2 ON ta.ChildTableVID=tv2.TableVID 
  JOIN [KeyHeaderMapping] khm ON khm.AssociationID=ta.AssociationID 
  JOIN [TableVersionHeader] tvh  ON ta.ParentTableVID=tvh.TableVID AND khm.PrimaryKeyHeaderID=tvh.HeaderID 
  JOIN [TableVersionHeader] tvh2 ON ta.ChildTableVID=tvh2.TableVID AND khm.ForeignKeyHeaderID=tvh2.HeaderID  
  JOIN [HeaderVersion] hv  ON hv.HeaderVID=tvh.HeaderVID 
  JOIN [HeaderVersion] hv2 ON hv2.HeaderVID=tvh2.HeaderVID 
  JOIN [ItemCategory] ic ON ic.ItemID=hv.PropertyID 
  JOIN [PropertyCategory] pc ON pc.PropertyID=hv.PropertyID 
  JOIN [Category] c ON pc.CategoryID=c.CategoryID 
  JOIN [PropertyCategory] pc2 ON pc2.PropertyID=hv2.PropertyID 
  WHERE Ta.StartReleaseID=@CurrentRelease AND ic.EndReleaseID is Null and pc.EndReleaseID is Null AND pc2.EndReleaseID is NULL 
  AND pc2.CategoryID<>pc.CategoryID


-- 98:  In any table association that has end release = null, it must have keyHeaderMapping for all the primary keys of the parent table
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 9999   --1010000026
  SELECT DISTINCT 
		 '1_24'																										AS ViolationCode, 
		 'The Header mapping in this table association'+ta.Name+' does not include all the Primary Key Headers and only them'	AS Violation,
		 1																											AS isBlocking,
		 tv.TableVID																								AS TableVID, 
		 NULL																										AS OldTableVID,
		 tv.Code		    																						AS TableCode, 
  ...
  FROM [TableAssociation] ta 
  JOIN [TableVersion]  tv ON ta.ParentTableVID=tv.TableVID 
  WHERE Ta.StartReleaseID=@CurrentRelease 
  AND (
		EXISTS	   (SELECT tvh.HeaderID 
					FROM TableVersionHeader tvh 
					JOIN Header h ON tvh.HeaderID=h.HeaderID and h.IsKey=1 
					WHERE tvh.TableVID=tv.TableVID AND 
					tvh.HeaderID NOT IN (SELECT khm.PrimaryKeyHeaderID FROM KeyHeaderMapping khm WHERE khm.AssociationID=ta.AssociationID))
  OR 
		EXISTS	   (SELECT tvh.HeaderID 
					FROM TableVersionHeader tvh 
					JOIN Header h ON tvh.HeaderID=h.HeaderID and h.IsKey=0 
					WHERE tvh.TableVID=tv.TableVID AND 
					tvh.HeaderID IN (SELECT khm.PrimaryKeyHeaderID FROM KeyHeaderMapping khm WHERE khm.AssociationID=ta.AssociationID))
		     )
...
-- 99. Attention to Main Properties defined of type 'es' that is string including the empty string
  INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int =  5
  SELECT DISTINCT 
		 '3_10'														AS ViolationCode, 
		 'WARNING: Main Property assigned to Header belongs to empty-string allowed data type es' AS Violation,
		 0															AS isBlocking,
		 tv.TableVID												AS TableVID, 
		 NULL														AS OldTableVID,
		 tv.Code													AS TableCode, 
  ...
  JOIN [DataType] dt ON dt.DataTypeID=p.DataTypeID  
  WHERE  mv.StartReleaseID = @CurrentRelease AND dt.Code='es' 
  ORDER BY tv.TableVID, h.Direction, hv.Code;



-- 100. A SubCategory Updated in CurrentRelease cannot contain exactly the same Items the previous SubCategoryVersion
 INSERT INTO ModelViolations 
  --DECLARE @CurrentRelease int = 9999 --1010000092
        SELECT --DISTINCT 
		 '6_36'																										AS ViolationCode, 
		 'SubCategory: '+trim(left(sc.Code,20))+' Updated in release: '+r.Code+' contains exactly the same SubcategoryItems as the previous SubCategoryVersion '		AS Violation,
		 1																											AS isBlocking,
		 NULL																										AS TableVID, 
		 NULL																										AS OldTableVID,
		 NULL																										AS TableCode, 
  ...
  INNER JOIN Category c on c.CategoryID=sc.CategoryID 
  inner join SubCategoryVersion SCV on SC.SubCategoryID=SCV.SubCategoryID 
  inner join SubCategoryVersion scv2 on scv2.SubCategoryID=sc.SubCategoryID  
  inner join Release r ON scv.StartReleaseID=r.ReleaseID 
  inner join Release r2 ON scv2.StartReleaseID=r2.ReleaseID 
  WHERE  
    scv2.subcategoryvid<scv.subcategoryvid AND 
	scv.SubCategoryVID<>scv2.SubCategoryVID AND 
	scv.EndReleaseID is NULL 
	and scv.StartReleaseID = @CurrentRelease 
	and scv2.EndReleaseID =  @CurrentRelease 
   AND (SELECT count(*) FROM SUbCategoryItem sci WHERE sci.SubCategoryVID=scv.SubCategoryVID) = 
  (SELECT count(*) FROM SUbCategoryItem sci INNER JOIN SubCategoryItem sci2 on sci.ItemID=sci2.ItemID 
    WHERE sci.SubCategoryVID=scv.SubCategoryVID AND sci2.SubCategoryVID=scv2.SubCategoryVID)
  AND (SELECT count(*) FROM SUbCategoryItem sci WHERE sci.SubCategoryVID=scv.SubCategoryVID) = 
